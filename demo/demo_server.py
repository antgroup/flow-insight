...
async def simulate_call_chain(client, flow_id, event_count):
    ...
    context_event = ContextEvent(
        ...
        context={"request_id": str(uuid.uuid4()), "user_id": f"user-{random.randint(1, 1000)}", "timestamp": timestamp}
    )
...
async def emit_node_physical_stats(client):
    ...
    while True:
        ...
        event = BatchNodePhysicalStatsEvent(stats=batch_stats)
...
async def emit_prompt_register(client):
    ...
    while True:
        ...
        prompt_event = PromptRegisterEvent(prompt=random.choice(prompts))
