...
const extractResourceUsage = (
  ...
  const matchingKey = Object.keys(resources).find(key => {
    ...
    return type.toLowerCase() === resourceType.toLowerCase() && id.toLowerCase() === pgIdLower;
  });
    ...
    // For CPU resources, calculate CPU usage from processStats or nodeCpuPercent or resource_usage
      ...
      Object.values(nodeData.services).forEach((service: Service) => {
        if (service?.placementGroup?.id?.toLowerCase() === pgIdLower) {
          ...
          else if (service.nodeCpuPercent !== undefined && !hasResourceUsageInfo) {
...
const getAvailableResources = (
  ...
  customResources.forEach(resource => {
    ...
    if (!resourceTypes.some(r => r.key.toLowerCase() === resource.toLowerCase())) {
      resourceTypes.push({
        key: resource,
...
const getAvailableContextKeys = (physicalViewData: PhysicalViewData) => {
      ...
      if (nodeData?.services) {
        Object.values(nodeData.services).forEach((service: any) => {
          if (service?.contextInfo) {
            Object.keys(service.contextInfo).forEach(key => contextKeys.add(key));
          }
        });
      }
    });
  ...
  return Array.from(contextKeys)
    ...
    .map(key => ({
      key,
      label: key === 'instance_id' ? 'Instance ID' : key === 'service_name' ? 'Service Name' : key,
    }));
};
...
const getContextColorScale = (values: string[]) => {
  const colorScale = d3.scaleOrdinal<string>().domain(values).range(d3.schemeCategory10);
  return colorScale;
};
...
const hasResourceInfo = (data: Service | NodeData, resourceType: string): boolean => {
  if (resourceType.toLowerCase() === 'gpu') {
    ...
    if ((data as Service).gpuDevices) {
      return (data as Service).gpuDevices?.some(gpu => gpu.utilization !== undefined) ?? false;
    }
    if ((data as NodeData).gpus) {
      return (data as NodeData).gpus?.some(gpu => gpu.utilization !== undefined) ?? false;
    }
    if ((data as NodeData).services) {
      return Object.values((data as NodeData).services).some(
        service => service.gpuDevices?.some(gpu => gpu.utilization !== undefined) ?? false
      );
    }
...
const PhysicalVisualization = forwardRef<PhysicalVisualizationHandle, PhysicalVisualizationProps>(
  ({ physicalViewData, onElementClick, selectedElementId, searchTerm = '' }, ref) => {
    ...
    const [selectedContext, setSelectedContext] = useState<string>('service_name');
    ...
    const serviceMatchesSearch = useCallback(
      (service: Service): boolean => {
        ...
        // Check instance ID
        if (service.instanceId && service.instanceId.toLowerCase().includes(searchTermLower)) {
          return true;
        }
    ...
    const serviceMatchesContextFilter = useCallback(
      (service: Service): boolean => {
        ...
        return contextValue !== undefined && contextValue.toLowerCase().includes(filterLower);
      },
    ...
    const addGlowFilter = useCallback(
      (svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
        ...
        filter
          ...
          .attr('in', 'blur')
          .attr('type', 'matrix')
          .attr('values', '0 0 0 0 0.298 0 0 0 0 0.686 0 0 0 0 0.314 0 0 0 1 0');
    ...
    const renderPhysicalView = useCallback(() => {
      ...
      nodes.forEach(([_, nodeData]) => {
        ...
        Object.values(placementGroups).forEach(services => {
          ...
          const pgWidth = services.length * SERVICE_WIDTH + (services.length - 1) * 8 + 40; // Add margins and padding
      ...
      nodes.forEach(([_, nodeData]) => {
        ...
        const pgTotalHeight =
          pgKeys.length * (serviceBoxHeight + serviceResourceBarMargin + groupSpacing);
        ...
        const totalNodeHeight = headerHeight + pgTotalHeight + freeServicesHeight + topMargin;
      ...
      nodes.forEach(([nodeId, nodeData], index) => {
        ...
        if (contextValueFilter && contextValueFilter.trim() !== '') {
          const hasMatchingServices = Object.values(nodeData?.services || {}).some(service =>
            serviceMatchesContextFilter(service)
          );
        ...
        } else if (selectedResource && selectedResource.toLowerCase() === 'gpuutil') {
        ...
        const freeServicesY =
          headerHeight + (pgKeys.length > 0 ? pgKeys.length * (pgTotalHeight + groupSpacing) : 0);
    ...
    const exportSvg = () => {
      ...
      a.href = url;
      a.download = `physical-visualization-${new Date().toISOString().slice(0, 10)}.svg`;
    ...
    return (
      ...
      >
        <div className="graph-container" style={{ flex: '1 1 auto', position: 'relative' }}>
          ...
          >
            <FormControl size="small" style={{ width: '150px' }}>
              ...
              >
                {getAvailableResources(
                  physicalViewData.physicalView?.[Object.keys(physicalViewData.physicalView)[0]]
                    ?.resources || {},
                  physicalViewData
                ).map(({ key, label }) => (
            ...
            <FormControl size="small" style={{ width: '150px' }}>
              ...
              >
                {getAvailableContextKeys(physicalViewData).map(({ key, label }) => (
                  <MenuItem key={key} value={key}>
                    {label}
                  </MenuItem>
                ))}
...
const renderPlacementGroups = (
  ...
  pgKeys.forEach((pgId, pgIndex) => {
    ...
    if (selectedResource) {
      resourceInfo = extractResourceUsage(nodeData.resources, pgId, selectedResource, nodeData);
    }
    ...
    services.forEach((service: Service, serviceIndex: number) => {
      ...
      if (selectedResource) {
        if (selectedResource.toLowerCase() === 'gpu' && hasResourceInfo(service, 'gpu')) {
        ...
        } else if (selectedResource.toLowerCase() === 'cpu' && hasResourceInfo(service, 'cpu')) {
...
const getResourceUsageFromField = (service: Service, resourceType: string): any => {
  ...
  if (baseResource.toLowerCase() === 'gpu' && service.gpuDevices && service.gpuDevices.length > 0) {
