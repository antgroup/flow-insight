...
import { FlameGraphData, PhysicalViewData, DebugSession, GraphData, Method } from '../types';
...
const Visualization = forwardRef<VisualizationHandle, VisualizationProps>(
  (
    {
      graphData,
      physicalViewData,
    ...
    const [activeDebugSessions, setActiveDebugSessions] = useState<DebugSession[]>([]);
    ...
    const exportSvg = () => {
      ...
      a.href = url;
      a.download = `visualization-${viewType}-${new Date().toISOString().slice(0, 10)}.svg`;
      ...
      if (selectedElementId) {
        ...
        if (nodeData) {
          ...
          svg.selectAll('text').each(function () {
            ...
            if (classes.includes('edge-label') || classes.includes('edgeLabel')) {
    ...
    const renderCircularSubgraphLayout = (
      ...
      const hasActiveDebugSession = (activeDebugSessions: DebugSession[], node: any): boolean => {
        ...
        return activeDebugSessions.some(session => {
          if (nodeType === 'method' && session.serviceInfo && session.serviceInfo.length > 0) {
            return node.instanceId === session.serviceInfo[1] && node.name === session.funcName;
      ...
      const calculateLineWidth = (speedValue: number, minSpeed: number, maxSpeed: number) => {
        ...
        const normalizedSpeed = (logSpeed - logMinSpeed) / (logMaxSpeed - logMinSpeed);
      ...
      // Find the main node
      const mainNode = graphData.functions.find(func => func.id === '_main') || {
        id: '_main',
        name: '_main',
      };
      ...
      subgraphs.forEach(subgraph => {
        ...
        subgraph.forEach(nodeId => {
          ...
          // Find the node data
          const serviceData = graphData.services.find(service => service.id === nodeId);
          if (serviceData) {
            ...
            if (methodData) {
              ...
              const service = graphData.services.find(s => s.id === methodData.instanceId);
          ...
          if (nodeData) {
            ...
            subG.setNode(nodeId, {
              label: label + (hasActiveDebugSession(activeDebugSessions, nodeData) ? ' 🐞' : ''),
              class: nodeType,
              style: style,
              rx: 5,
              ry: 5,
        ...
        graphData.dataFlows.forEach((flow, i) => {
          if (subgraph.includes(flow.source) && subgraph.includes(flow.target)) {
            const speedValue = getSpeedValue(flow);
            const lineWidth = calculateLineWidth(speedValue, minSpeed, maxSpeed);
      ...
      mainNodeContainer
        .append('circle')
        .attr('r', 60)
        .attr('fill', colorScheme.function)
        .attr('stroke', '#333')
        .attr('stroke-width', 3);
      ...
      subgraphLayouts.forEach((subLayout, index) => {
        ...
        const subContainer = mainContainer.append('g').attr('transform', `translate(${x},${y})`);
        ...
        subLayout.nodes.forEach(nodeId => {
          ...
          if (node && node.originalData) {
            ...
            node.style = baseStyle.replace(/fill: ([^;]+);/, `fill: $1; opacity: ${opacity};`);
          }
        });
      ...
      dataFlowsByArgpos.forEach((argposMap, key) => {
        ...
        argposMap.forEach((flows, argpos) => {
          ...
          if (!mergedDataFlows.has(key)) {
            ...
            mergedFlow.duration = Math.max(mergedFlow.duration || 0, latestFlow.duration || 0);
        ...
        edges.forEach((edge, groupIndex) => {
          ...
          if (sourceNode && targetNode) {
            ...
            if (
              ...
              flow.target === '_main' ||
              !subgraphs.some(
                subgraph => subgraph.includes(flow.source) && subgraph.includes(flow.target)
              )
            ) {
              // Calculate offset for multiple edges between the same nodes
              // Only apply offset if there are multiple edges
              const edgeOffset = edges.length > 1 ? (groupIndex - (edges.length - 1) / 2) * 15 : 0;
              ...
              mainContainer
                .append('path')
                .attr('d', () => {
                  let sourceX = flow.source === '_main' ? centerX : sourceNode.x;
                  let sourceY = flow.source === '_main' ? centerY : sourceNode.y;
                  let targetX = flow.target === '_main' ? centerX : targetNode.x;
                  let targetY = flow.target === '_main' ? centerY : targetNode.y;
                  ...
                  if (flow.source === '_main' || flow.target === '_main') {
                    ...
                    const curvePointX =
                      centerX + (fromCenterX / distFromCenter) * distFromCenter * factor;
                    const curvePointY =
                      centerY + (fromCenterY / distFromCenter) * distFromCenter * factor;
                ...
                .attr('marker-end', isDataFlow ? 'url(#redarrowhead)' : 'url(#arrowhead)')
        ...
        label
          .attr('x', () => {
            ...
            if (flow.source === '_main' || flow.target === '_main') {
              ...
              const updatedSourceX = flow.source === '_main' ? mainNodeX : sourceX;
              const updatedSourceY = flow.source === '_main' ? mainNodeY : sourceY;
              const updatedTargetX = flow.target === '_main' ? mainNodeX : targetX;
              const updatedTargetY = flow.target === '_main' ? mainNodeY : targetY;

              // Calculate control points for the curve - using the same logic as the edge path
              const curveStrength = 0.5;
              const midX = updatedSourceX + (updatedTargetX - updatedSourceX) * 0.5;
          ...
          .attr('y', () => {
            ...
            if (flow.source === '_main' || flow.target === '_main') {
              ...
              // Calculate control points for the curve - using the same logic as the edge path
              const curveStrength = 0.5;
              const midY = updatedSourceY + (updatedTargetY - updatedSourceY) * 0.5;
      ...
      const highlightRelatedEdges = (nodeId: string) => {
        ...
        // For subgraph edges
        mainContainer.selectAll('g > g > g.edgePath path').style('opacity', 0.1);
          ...
          if (edgeData && edgeData.v && edgeData.w) {
            ...
            if (isConnected) {
              ...
              mainContainer.selectAll('g.edgeLabel').each(function () {
                ...
                if (labelData && labelData.w === edgeData.w && labelData.v === edgeData.v) {
      ...
      const mainNodeCircle = mainContainer.select('.main-node-container circle');
    ...
    const updateParnetRef = (inner: any) => {
      if (inner && !inner.empty()) {
        ...
        const match = innerTransform?.match(/translate\(([^,]+),([^)]+)\) scale\(([^)]+)\)/);
