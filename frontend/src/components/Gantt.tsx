    ...
    (
        ...
        const transformFlameDataToTreeTasks = (data: FlameGraphData): CustomTask[] => {
            ...
            const parseServiceMethod = (
                ...
                // Keep track of span information for display uniqueness
                let spanInfo = '';
                let cleanName = fullName;

                // Extract span suffix if present (everything after the last @)
                const lastAtIndex = cleanName.lastIndexOf('@');
                if (lastAtIndex !== -1) {
                    spanInfo = cleanName.substring(lastAtIndex + 1);
                    cleanName = cleanName.substring(0, lastAtIndex);

                    // If it's our added span context, make it more readable
                    if (spanInfo.startsWith('span')) {
                        spanInfo = ` #${spanInfo.replace('span', '').replace('_', '-')}`;
                    } else {
                        // Keep original span ID but make it shorter for display
                        spanInfo = ` @${spanInfo.substring(0, 8)}`;
                    }
                }
                ...
                if (match) {
                    ...
                    return {
                        serviceName,
                        methodName,
                        displayName: `${serviceName}.${methodName}${spanInfo}`,
                    };
                }
                ...
                if (parts.length >= 2) {
                    return {
                        ...
                        displayName: `${parts[0]}.${parts[parts.length - 1]}${spanInfo}`,
                    };
                }

                return { serviceName: 'Unknown', methodName: cleanName, displayName: `${cleanName}${spanInfo}` };
            };
            ...
            const convertHierarchyToTasks = (
                ...
                // Generate unique ID using full name including span information
                const uniqueId = `${node.name}_${level}_${flatTasks.length}_${startTime}_${Date.now()}`;
                ...
                const task: CustomTask = {
                    ...
                    fullName: node.name, // Keep full name with span info for tracking
                    startTime: startTime,
                    endTime: endTime,
                    progress: 100,
                    type: 'method',
                ...
                <div style={{ marginBottom: '24px' }}>
                    ...
                    >
                        ...
                        <span style={{ fontStyle: 'italic', color: COLORS.textLight }}>
                            💡 Click tasks to inspect • Hover for details • Zoom for precision • Use ⬆ to flatten bars
                        </span>
                    </div>
                </div>
                ...
                >
                    ...
                    >
                        ...
                        {filteredTasks.map((task, index) => {
                            ...
                            let baseTaskStartX = labelWidth + timeToX(task.startTime, minTime, pixelsPerUnit, timeScale);
                            let baseTaskEndX = labelWidth + timeToX(task.endTime, minTime, pixelsPerUnit, timeScale);

                            // Apply flattening to bar positions
                            const getAdjustedBarPosition = (task: CustomTask): { startX: number; endX: number } => {
                                ...
                                const calculateFlattenedPosition = (taskId: string): { baseStartX: number; foundFlattened: boolean } => {
                                    ...
                                    if (flattenedGroups.has(parentId)) {
                                        ...
                                        if (parentTask) {
                                            const parentBaseStartX = labelWidth + timeToX(parentTask.startTime, minTime, pixelsPerUnit, timeScale);
                                ...
                                <g key={task.id}>
                                    ...
                                    <g>
                                        ...
                                        >
                                            {task.name}
                                            {flattenedGroups.has(task.id) && (
                                                <tspan fill={COLORS.accent} fontWeight="bold"> [BARS FLATTENED]</tspan>
                                            )}
                                        </text>
