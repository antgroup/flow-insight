    ...
    (
        ...
        const transformFlameDataToTreeTasks = (data: FlameGraphData): CustomTask[] => {
            ...
            const addFunctionToList = (
                ...
                // Use specific timing if provided, otherwise calculate from executions
                let functionStartTime: number;
                let functionEndTime: number;

                if (specificStartTime !== undefined && specificEndTime !== undefined) {
                    // Use the specific timing for this parent-child context
                    functionStartTime = specificStartTime;
                    functionEndTime = specificEndTime;
                } else {
                    // Calculate function timing from all executions (fallback)
                    functionStartTime = Infinity;
                    functionEndTime = 0;
                ...
                // Use specific count if provided, otherwise use total executions
                const executionCount = specificCount !== undefined ? specificCount : functionNode.executions.length;

                // Generate unique ID to prevent functions with same name from being grouped
                const uniqueId = `${functionNode.id}_${level}_${flatTasks.length}_${functionStartTime}`;

                // Add function task
                const functionTask: CustomTask = {
                    id: uniqueId,
                    name: `${functionNode.name} (${executionCount}x)`,
                ...
                sortedCallees.forEach(calleeNode => {
                    // Find specific timing and count for this callee when called by this parent
                    let childStartTime: number | undefined;
                    let childEndTime: number | undefined;
                    let childCount: number | undefined;

                    // Look for timing data in the callee's totalInParent that matches current function
                    if (calleeNode.totalInParent) {
                        const parentRelation = calleeNode.totalInParent.find(
                            relation => relation.callerNodeId === functionNode.fullName
                        );
                        if (parentRelation) {
                            childStartTime = parentRelation.startTime;
                            childEndTime = parentRelation.startTime + (parentRelation.duration * 1000);
                            childCount = parentRelation.count;
                        }
                    }

                    // If no specific timing found, use the callee's first execution
                    if (childStartTime === undefined && calleeNode.executions.length > 0) {
                        childStartTime = calleeNode.executions[0].startTime;
                        childEndTime = calleeNode.executions[0].endTime;
                    }
            ...
            if (mainNode.children.size > 0) {
                ...
                sortedMainChildren.forEach((rootFunction, index) => {
                    addFunctionToList(rootFunction, 1, index, undefined); // No parent for root level functions
                });
            }
