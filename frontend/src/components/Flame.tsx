...
const convertFlameTreeToFlameNode = (treeNode: FlameTreeNode, currentTimestamp: number): FlameNode => {
    const convertNode = (node: FlameTreeNode): FlameNode => {
        ...
        if (node.id === 'root' || node.id === '_main' || node.endTime === -1) {
            originalValue = children ? children.reduce((sum, child) => sum + (child.originalValue || 0), 0) : 0.001;
            isRunning = false;
        } else {
        ...
        if (node.children && node.children.length > 0) {
            ...
            if (visibleChildren.length > 0) {
                ...
                visibleChildren.forEach((child, index) => {
                    const childWidth = totalChildValue > 0 ? (child.value / totalChildValue) * availableWidth : availableWidth / visibleChildren.length;
...
// Zoom functionality
const zoomToNode = (root: LayoutNode, targetNode: LayoutNode, width: number, cellHeight: number): LayoutNode => {
...
const FlameVisualization = forwardRef<FlameVisualizationHandle, FlameVisualizationProps>(
    (
        ...
        const showTooltip = (node: LayoutNode, event: MouseEvent) => {
            ...
            const formattedValue = valueInSeconds < 0.000001 ? valueInSeconds.toExponential(6) : valueInSeconds.toFixed(6);
        ...
        const updateTooltipPosition = (event: MouseEvent) => {
            ...
            const wouldBeCloseToRightEdge = event.clientX + tooltipWidth + offset.x > screenRightEdge - 400;
            ...
            const wouldBeCloseToBottomEdge = event.clientY + tooltipHeight + offset.y > screenBottomEdge - 400;
        ...
        const handleZoomClick = (node: LayoutNode) => {
            ...
            const isMainNode = node.data.name === '_main' || node.data.name === 'root' ||
                node.data.name === 'main' || node.data.name.includes('main');
            ...
            const findNodeInOriginal = (searchNode: LayoutNode, targetName: string, targetDepth: number): LayoutNode | null => {
            ...
            if (containerRef.current && originalLayoutRef.current) {
                const width = containerRef.current.clientWidth;
                const originalNode = findNodeInOriginal(originalLayoutRef.current, node.data.name, node.depth);

                if (originalNode) {
                    const zoomedLayout = zoomToNode(originalLayoutRef.current, originalNode, width, cellHeight);
