...
export class ApiService {
  ...
  public async request<T>(
    ...
    if (method === 'GET') {
      ...
      const requestPromise = this.makeRequest<T>(url, options, data)
        .then(result => {
  ...
  private ATStatsToService(
    contextData: ATContext,
    resourceUsage: ATResourceUsage,
    nodeStats: ATNode,
    serviceStats: ATServiceStats,
    instance_id: string,
    servce_name: string
  ): Service {
    return {
      ...
      nodeMem: [
        nodeStats.memoryInfo.total,
        nodeStats.memoryInfo.available,
        0,
        nodeStats.memoryInfo.available,
      ],
      placementGroup: {
  ...
  public async getGraphData(
    ...
    const graphData: GraphData = {
      services: apiGraphData.services.map((service: ATService) =>
        this.ATStatsToService(
          contextDataMap[service.instanceId],
          resourceUsageMap[service.instanceId],
  ...
  public async getPhysicalViewData(flowId: string, end_time?: number): Promise<PhysicalViewData> {
    ...
    const serviceIdToName: Record<string, string> = serviceStats.reduce(
      (acc, service) => {
        ...
        Object.entries(nodeStats).map(([nodeId, nodeData]) => [
          nodeId,
          {
            ...
            services: Object.fromEntries(
              Object.entries(serviceStatsMap)
                .filter(([instanceId, serviceData]) => serviceData.nodeId === nodeId)
                .map(([instanceId, serviceData]) => [
                  instanceId,
                  this.ATStatsToService(
                    contextDataMap[instanceId],
                    resourceUsageMap[instanceId],
                    nodeData,
                    serviceData,
                    instanceId,
                    serviceIdToName[instanceId]
                  ),
                ])
            ),
  ...
  public async deactivateDebugSession(flowId: string, spanId: string): Promise<boolean> {
