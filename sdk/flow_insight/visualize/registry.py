...
def register_module(module: types.ModuleType, module_name: str = None) -> None:
    ...
    _instrumented_modules[module_name] = {
        'module': module,
        'original_methods': {}
    }
...
def unregister_module(module_name: str) -> None:
    ...
    for name, obj in inspect.getmembers(module):
        if inspect.isclass(obj):
            for method_name, original_method in module_info['original_methods'].get(name, {}).items():
...
def _instrument_function(module: types.ModuleType, func_name: str, func: Any, module_name: str) -> None:
    """Instrument a module-level function."""
    
    module_info = _instrumented_modules[module_name]

    instrumented_func = create_instrumented_method(
        func, None, func_name
    )
