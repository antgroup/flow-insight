...
from flow_insight.model import CallSubmitEvent, ObjectGetEvent, ObjectPutEvent, ContextEvent, ResourceUsageEvent, CallEndEvent, CallBeginEvent, DebuggerInfoEvent, BatchServicePhysicalStatsEvent, BatchNodePhysicalStatsEvent, PromptRegisterEvent
from flow_insight.storage.snapshot.model import Service, Method, ObjectEvent, ObjectInfo, Context, ResourceUsage, FlameDataAggregated, DebuggerInfo, CallerInfo, Breakpoint, NodePhysicalStats
...
class InsightEngine:
    ...
    async def get_debug_sessions(self, flow_id: str, service_name: Optional[str], instance_id: Optional[str], method_name: Optional[str], filter_active: bool = False):
        if service_name is None and instance_id is None and method_name is None:
            ...
            return ret
        ret = []
        service = Service(service_name=service_name, instance_id=instance_id) if service_name else None
    ...
    async def activate_debug_session(self, flow_id: str, service_name: Optional[str], instance_id: Optional[str], method_name: Optional[str], span_id: str):
        service = Service(service_name=service_name, instance_id=instance_id) if service_name else None
        method = Method(name=method_name)
        debugger_info = await self._snapshot_storage.get_debugger_info(flow_id, service, method, span_id)
    ...
    async def debug_cmd(self, flow_id: str, span_id: str, command: DebugCommand, args: Dict[str, Any]):
    ...
    async def emit_call_submit(self, call_submit: CallSubmitEvent):
        ...
        source_service = Service(service_name=call_submit.source_service, instance_id=call_submit.source_instance_id) if call_submit.source_service else None
        source_method = Method(name=call_submit.source_method)
        target_service = Service(service_name=call_submit.target_service, instance_id=call_submit.target_instance_id) if call_submit.target_service else None
        target_method = Method(name=call_submit.target_method)
        start_time = call_submit.timestamp

        await self._snapshot_storage.set_start_time(flow_id, source_service, source_method, target_service, target_method, start_time)
        await self._snapshot_storage.update_flow_record(flow_id, source_service, source_method, target_service, target_method, lambda record: record + 1)
        await self._snapshot_storage.update_call_graph(flow_id, source_service, source_method, target_service, target_method, lambda record: {"count": record.get("count", 0) + 1, "start_time": start_time})
    ...
    async def emit_object_get(self, object_get: ObjectGetEvent):
        ...
        object_event: ObjectEvent = await self._snapshot_storage.get_object_events(flow_id, object_id)
        caller_service = object_event.sender_service
        caller_method = object_event.sender_method
        callee_service = Service(service_name=object_get.receiver_service, instance_id=object_get.receiver_instance_id) if object_get.receiver_service is not None else None
        ...
        await self._snapshot_storage.update_data_flow(flow_id, caller_service, caller_method, callee_service, callee_method, ObjectInfo(size=size, argpos=argpos, duration=duration, timestamp=timestamp))
    ...
    async def emit_object_put(self, object_put: ObjectPutEvent):
        ...
        service = Service(service_name=object_put.sender_service, instance_id=object_put.sender_instance_id) if object_put.sender_service is not None else None
        method = Method(name=object_put.sender_method)
        object_event = ObjectEvent(timestamp=object_put.timestamp, object_id=object_put.object_id, sender_service=service, sender_method=method, object_info=ObjectInfo(size=object_put.object_size, argpos=object_put.object_pos, duration=0, timestamp=object_put.timestamp))
    ...
    async def emit_context(self, context_add: ContextEvent):
        ...
        if context_add.service_name is not None:
            service = Service(service_name=context_add.service_name, instance_id=context_add.instance_id)
    ...
    async def emit_resource_usage(self, resource_usage: ResourceUsageEvent):
        ...
        if resource_usage.service_name is not None:
            service = Service(service_name=resource_usage.service_name, instance_id=resource_usage.instance_id)
    ...
    async def get_flame_graph_data(self, flow_id):
        ...
        for func_id , func_data in flame_graph_aggregated.items():
            ...
            start_times = await self._snapshot_storage.get_start_time(flow_id, func_id[0], func_id[1])
            for current_span_id, duration in func_data.durations.items():
                caller_infos = await self._snapshot_storage.get_caller_info(flow_id, current_span_id)
                for caller_info in caller_infos:
                    ...
                    if caller_service is not None:
                        caller_node_id = (
                            f"{caller_service.service_name}:{caller_service.instance_id}.{caller_method.name}"
                        )
                    else:
            ...
            flame_data.aggregated.append(
                AggregatedFlameGraphData(
                    name=method.name if service is None else f"{service.service_name}:{service.instance_id}.{method.name}",
        ...
        for callee_id, start_times in start_times.items():
            if callee_id not in visited:
                ...
                str_id = (
                    f"{callee_service.service_name}:{callee_service.instance_id}.{callee_method.name}"
    ...
    async def emit_call_end(self, call_end: CallEndEvent):
        ...
        target_service = Service(service_name=call_end.target_service, instance_id=call_end.target_instance_id) if call_end.target_service is not None else None
        target_method = Method(name=call_end.target_method)
        span_id = call_end.span_id
        await self._snapshot_storage.del_debugger_info(flow_id, target_service, target_method, span_id)

        caller_infos = await self._snapshot_storage.get_caller_info(flow_id, span_id)
        for caller_info in caller_infos:
            await self._snapshot_storage.update_flow_record(flow_id, caller_info.service, caller_info.method, target_service, target_method, lambda record: record - 1)

        duration = call_end.duration

        await self._snapshot_storage.update_flame_graph_data(flow_id, target_service, target_method, lambda flame_data: FlameDataAggregated(total_time=flame_data.total_time + duration, call_count=flame_data.call_count + 1, durations={**flame_data.durations, span_id: duration}, service_name=flame_data.service_name))

    async def emit_debugger_info(self, debugger_info: DebuggerInfoEvent):
        flow_id = debugger_info.flow_id
        service = Service(service_name=debugger_info.service_name, instance_id=debugger_info.instance_id) if debugger_info.service_name else None
        ...
        await self._snapshot_storage.set_debugger_info(flow_id, service, method, span_id, DebuggerInfo(debugger_host=debugger_host, debugger_port=debugger_port, debugger_enabled=debugger_enabled))

    async def emit_call_begin(self, call_begin: CallBeginEvent):
        flow_id = call_begin.flow_id
        span_id = call_begin.span_id
        service = Service(service_name=call_begin.source_service, instance_id=call_begin.source_instance_id) if call_begin.source_service else None
        method = Method(name=call_begin.source_method)
        await self._snapshot_storage.add_caller_info(flow_id, span_id, CallerInfo(service=service, method=method))

    async def emit_service_physical_stats(self, service_physical_stats: BatchServicePhysicalStatsEvent):
        flow_id = service_physical_stats.flow_id
        await self._snapshot_storage.batch_add_service_physical_stats(flow_id, service_physical_stats.stats)
